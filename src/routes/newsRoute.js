...
router.get('/', async (req, res, next) => {
	...
	if (title) {
		...
		newsTitle.length ? res.status(200).json(newsTitle) : res.status(200).json('No se encontraron resultados');
...
router.get('/:id', async (req, res, next) => {
	...
	newsId.length > 0 ? res.status(200).send(newsId) : res.status(200).send('Not found');
});
